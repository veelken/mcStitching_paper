...
proceeds by first simulating one ``hard-scatter'' interaction within a given range in $\pThat$
...
We refer to these additional inelastic $\Pp\Pp$ scattering interactions as ``pileup''
and use the symbol $N_{\pileup}$ to denote the total number of inelastic $\Pp\Pp$ scattering interactions (including the hard-scatter interaction) that occur in the same crossing of the proton beams.
No selection on $\pThat$ is applied when simulating the pileup interactions.
The distinction between the hard-scatter interaction and the pileup interactions is artificial and made for the purpose of MC production only.
...
In the data, the scattering in which the transverse momentum exchange between the protons amounts to $\pThat$ may occur in any of the $N_{\pileup}$ simultaneous $\Pp\Pp$ interactions.
...
The probability $P^{I} = P^{i_{1},\dots,i_{N_{\pileup}}}$ follows a {\em multinomial distribution}.
...
\hline
\multirow{2}{20mm}{Sample} & \multirow{2}{*}{Number of events} & \multirow{2}{*}{Cross section [nb]$^{\dagger}$} & \multicolumn{5}{c}{Probabilities}               \\
                           &                                   &                                                 & $P^{0}$ & $P^{1}$ & $P^{2}$ & $P^{3}$ & $P^{4}$ \\
\hline
\hline
...
\end{tabular}
\end{center}
$^{\dagger}$ Computed at at LO accuracy in pQCD, then scaled to NNLO
\caption{
...
\begin{tabular}{l|c|c}
\hline
Sample                   & Number of events    & Cross section [pb]$^{\dagger}$ \\
\hline
\hline
...
\begin{tabular}{l|c|c}
\hline
Sample                 & Number of events    & Cross section [nb]$^{\dagger}$ \\
\hline
\hline
...
\end{tabular}
\end{center}
$^{\dagger}$ Computed at at NLO accuracy in pQCD using the program \MGvATNLO, then scaled to NNLO
\caption{
