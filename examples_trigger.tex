...
The ``phase space'' corresponding to a given event is specified by a vector $I=n_{1},\dots,n_{m}$ of dimension $m$.
...
to feature $n_{1}$ inelastic $\Pp\Pp$ scatterings that fall into the first interval in $\pThat$, $n_{2}$ into the second,$\dots$, and $n_{m}$ into the $m$-th 
...
Dividing both numerator and denominator on the right-hand side of Eq.~(\ref{eq:weight_tmp}) by $P^{I}$ and replacing the ratio $P_{j}^{I}/P_{\incl}^{I}$ by Eq.~(\ref{eq:P_ratio}) yields:
\begin{equation}
w^{I} = \frac{F}{\sum_{k=1}^{N_{\incl}} \, w_{\incl}^{k} + \sum_{j} \, \frac{n_{j}}{(N_{\pileup} + 1) \cdot p_{j}} \cdot \sum_{k=1}^{N_{j}} \, w_{j}^{k}} \, .
...
tailored to the case of estimating trigger rates instead of estimating event yields of signal or background processes.
...
The stitching weights $s^{I}$ are on average smaller for events that pass the trigger selection than for events that fail it,
...
Thus, the reduction in statistical uncertainties that one obtains by using the exclusive samples and applying the stitching weights 
becomes more pronounced after the trigger selection is applied.
